...
async function runMigration() {
  try {
    console.log('🔄 Début de la migration Secteur-Canal...\n');

    // Étape 1: Créer la table secteur_canal
    console.log('📋 Étape 1: Création de la table secteur_canal...');
    await prisma.$executeRawUnsafe(`
      CREATE TABLE IF NOT EXISTS \`secteur_canal\` (
        \`secteur_id\` INT NOT NULL,
        \`canal_id\` INT NOT NULL,
        PRIMARY KEY (\`secteur_id\`, \`canal_id\`),
        INDEX \`fk_secteur_canal_secteur_idx\` (\`secteur_id\`),
        INDEX \`fk_secteur_canal_canal_idx\` (\`canal_id\`),
        CONSTRAINT \`fk_secteur_canal_secteur\`
          FOREIGN KEY (\`secteur_id\`) REFERENCES \`secteur\` (\`id\`)
          ON DELETE CASCADE
          ON UPDATE CASCADE,
        CONSTRAINT \`fk_secteur_canal_canal\`
          FOREIGN KEY (\`canal_id\`) REFERENCES \`canal\` (\`id\`)
          ON DELETE CASCADE
          ON UPDATE CASCADE
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
    `);
    console.log('✅ Table secteur_canal créée\n');
    ...
    if (columns.length > 0) {
      ...
      const result = await prisma.$executeRawUnsafe(`
        INSERT IGNORE INTO \`secteur_canal\` (\`secteur_id\`, \`canal_id\`)
        SELECT \`id\`, \`canal_id\`
        FROM \`secteur\`
      ...
      const check = await prisma.$queryRawUnsafe(`
        SELECT 
          (SELECT COUNT(*) FROM secteur WHERE canal_id IS NOT NULL) as secteurs_avec_canal,
          (SELECT COUNT(*) FROM secteur_canal) as relations_migrees;
      `);
      console.log('📊 Vérification:');
      console.log(`   Secteurs avec canal: ${check[0].secteurs_avec_canal}`);
      console.log(`   Relations migrées: ${check[0].relations_migrees}\n`);

      if (check[0].secteurs_avec_canal === check[0].relations_migrees) {
    ...
    if (error.message.includes('already exists')) {
      console.log('\nℹ️  La table secteur_canal existe déjà. C\'est normal.');
