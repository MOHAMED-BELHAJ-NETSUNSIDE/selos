...
import { useSousRegions, useCreateSousRegion, useUpdateSousRegion, useDeleteSousRegion, type SousRegion } from '@/hooks/use-sous-regions';

export default function SousRegionsPage() {
  const { data: sousRegions = [], isLoading, error } = useSousRegions();
  const createMutation = useCreateSousRegion();
  const updateMutation = useUpdateSousRegion();
  const deleteMutation = useDeleteSousRegion();
  ...
  const [editing, setEditing] = useState<SousRegion | null>(null);
  const [nom, setNom] = useState('');

  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase();
    const items = q ? sousRegions.filter((s) => s.nom.toLowerCase().includes(q) || String(s.id).includes(q)) : sousRegions;
    const start = (page - 1) * limit;
    return { items: items.slice(start, start + limit), total: items.length, pages: Math.max(1, Math.ceil(items.length / limit)) };
  }, [sousRegions, search, page]);

  const openCreate = () => { setEditing(null); setNom(''); setIsOpen(true); };
  const openEdit = (s: SousRegion) => { setEditing(s); setNom(s.nom); setIsOpen(true); };
  ...
  const handleDelete = async (s: SousRegion) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-900">Sous-régions</h1>
          <p className="text-gray-600 dark:text-gray-900">Gérez les sous-régions</p>
        </div>
        ...
        <div className="flex items-center space-x-4 p-6 pb-2">
          <div className="flex-1">
            <div className="relative">
              ...
              <Input placeholder="Rechercher une sous-région..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pl-10" />
            </div>
          </div>
        </div>
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-8 text-gray-500">Aucune sous-région trouvée</div>
          ) : (
            <>
              <Table>
                <TableHeader>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>{editing ? 'Modifier' : 'Ajouter'} une sous-région</DialogTitle>
          </DialogHeader>
          <div className="space-y-2">
            <Label htmlFor="nom">Nom</Label>
            <Input id="nom" value={nom} onChange={(e) => setNom(e.target.value)} placeholder="Ex: Sous-région Centre" />
          </div>
          <DialogFooter>
